...
interface Props {
  step: CreateChatStep;
  onStepChange: (step: CreateChatStep) => void;
  onCancel: () => void;
  ...
  return (
    <div className="flex flex-col gap-4 min-h-0">
      <p className="text-sm text-content-muted">
        Add people to <span className="text-content">{name.trim()}</span> — or
              ...
              {candidates.map((u) => (
                ...
                >
                  <div className="min-w-0">
                    <p className="text-content text-sm truncate">{u.userName}</p>
                    {(u.name || u.surname) && (
